...
public:
    CString2(const char* sht)
    {
        __asm
        {
            ...
            SUB     7,      ESP     ; strlen("hello.") + 1  -- 7
            PUSH    0               ; char* str = 0         -- 11
            PUSH    0               ; size_t size = 0       -- 15
            PUSH    0               ; size_t length = 0     -- 19
            PUSH    1               ; bool sso = true       -- 23
            MOV     0,      ECX     ; size_t len = 0
            
        shrt:
            CMP     0,      EBP-ECX ; sht[len] != '\0'
            JNE     aftr
            MOV     1,      EAX     ; uint8_t x = 32 - (8 * (len + 1))
            ADD     ECX,    EAX
            MUL     8
            MOV     32,     EBX
            SUB     EAX,    EBX     ; EAX is not needed now
        ...
        _1:
            OR      EBX,    EBP-11  ; size |= x
            JMP     iter

        _2:
            OR      EBX,    EBP-15  ; length |= x
            JMP     iter
        
        _3:
            PUSH    EBP + 8         ; mangle(sht + 8)
            CALL    mangle
            JMP     iter            ; ++i
        ...
        for( ; sht[len] != '\0'; ++len)
        {
            uint8_t x = 32 - (8 * (len + 1));
            if(len < 4)       m_size  |= x;
            else if(len < 8)  m_length|= x;
            else if(len == 8) m_string = mangle(sht);
        ...
        // If "sso" is true and "m_string" is nullptr,
        // we have data in m_size and m_length, but we need
        // to move the remainder of the string data.
        if(sso && !m_string)
        {
            m_string = new char[len + 10];
            uint8_t i = unmangle(m_string, m_size);
            i += unmangle(m_string + i, m_length);
            m_length = len;
            m_size = len + 10;
            memcpy(m_string, sht + i, len - i);
        }
