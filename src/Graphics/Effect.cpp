...
CEffect::CEffect(const EffectType Type) : m_type(Type)
{
}
...
bool CEffect::Init()
{
    ...
    else
    {
        ...
        m_Log   << m_Log.SetMode(LogMode::ZEN_FATAL) << m_Log.SetSystem("Effect")
                << "Invalid effect type: " << m_type << "." << CLog::endl;
                           ...
                           const size_t count)
{
    ...
    if(count == 1)      glUniform1f(loc, pValues);
    else if(count == 2) glUniform2fv(loc, pValues, 1);
    else if(count == 3) glUniform3fv(loc, pValues, 1);
    else if(count == 4) glUniform4fv(loc, pValues, 1);
    else
    {
                           ...
                           const size_t count)
{
    ...
    if(count == 1)      glUniform1i(loc, pValues);
    else if(count == 2) glUniform2iv(loc, pValues, 1);
    else if(count == 3) glUniform3iv(loc, pValues, 1);
    else if(count == 4) glUniform4iv(loc, pValues, 1);
    else
    {
        m_Log   << m_Log.SetMode(LogMode::ZEN_ERROR)
                << m_Log.SetSystem("Effect")
